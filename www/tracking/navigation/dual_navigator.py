...
class DualNavigator(RootHolder):
    ...
    def categories_url(self, categories, task):
        return url_for(f'categories_bp.categories_{task}', place_id=self.place_id, particular_thing_id=self.particular_thing_id)
    ...
    def root_url(self, root, task, place_id=None, particular_thing_id=None):
        place_id = place_id or self.place_id or root.place.id
        particular_thing_id = particular_thing_id or self.particular_thing_id or root.thing.generic.id
        return url_for(f'root_bp.root_{task}', place_id=place_id, particular_thing_id=particular_thing_id)

    def thing_url(self, thing, task):
        if task == 'view':
            return self.root_url(thing.root, task, particular_thing_id=thing.generic.id)
        return url_for(f'thing_bp.thing_{task}', place_id=self.place_id, particular_thing_id=thing.generic.id)

    def particular_thing_url(self, particular_thing, task):
        if task == 'view':
            return self.root_url(particular_thing.root, task, particular_thing_id=particular_thing.id)
