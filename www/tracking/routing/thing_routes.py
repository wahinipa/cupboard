...
def thing_create(thing_id):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('thing_bp.thing_view', thing_id=new_thing.id))
    else:
        return thing.display_context(current_user).render_template(form=form,
...
def thing_delete(thing_id):
    ...
    if thing is not None and thing.may_delete(current_user):
        redirect_url = thing.url_on_delete
...
def thing_update(thing_id):
    ...
    if thing and thing.may_update(current_user):
        ...
        redirect_url = url_for('thing_bp.thing_view', thing_id=thing_id)
        if request.method == 'POST' and form.cancel_button.data:
            return redirect(redirect_url)
        if form.validate_on_submit():
            ...
            return thing.display_context(current_user).render_template('pages/form_page.j2', form=form)
    else:
        return redirect_hacks()
