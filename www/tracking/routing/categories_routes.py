...
@categories_bp.route('/create/<activity>/<int:place_id>/<int:thing_id>/<int:specification_id>', methods=['POST', 'GET'])
@login_required
def categories_create(**kwargs):
    return CategoriesCreateHandler(current_user, **kwargs).handle()


@categories_bp.route('/view/<activity>/<int:place_id>/<int:thing_id>/<int:specification_id>')
@login_required
